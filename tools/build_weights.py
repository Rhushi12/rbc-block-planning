...
def main():
    ...
    io.open(os.path.join(DIST, 'weights.js'), 'w', encoding='utf-8').write(
        ...
        % (len(train),
           ('Fitted to the block register in %s.' % os.path.basename(os.path.normpath(args.records)))
           if args.records else
           ('The history is SYNTHETIC pending access to a division\'s\n'
    ...
    for n, v, t in zip(NAMES, learned, truth):
        # Recorded choices have no generating weights to recover.
        if args.records:
            print('    %-16s learned %8.4f' % (n, v))
        else:
