...
def main():
    ...
    if args.records:
        try:
            ...
            records = real_records.validate(args.records,
                                            ['cycles.csv', 'decisions.csv'], cautions)
        ...
        for c in cautions:
            print('warning: %s' % c)
    source = ('recorded in %s' % os.path.basename(os.path.normpath(args.records))
              if records else 'SYNTHETIC')
    ...
    io.open(os.path.join(DIST, 'priority.js'), 'w', encoding='utf-8').write(
        ...
        % (len(hist),
           ('The history is %s.' % source) if records else
           ('The history is SYNTHETIC pending access to real TMS/SMMS/TDMS\n'
    ...
    io.open(os.path.join(DIST, 'hazard.js'), 'w', encoding='utf-8').write(
        ...
        '// Fitted by Newton-Raphson (IRLS) on %d asset cycles, %s.\n'
        ...
        % (len(h_train), source if records else 'synthetic', auc, brier, base) +
